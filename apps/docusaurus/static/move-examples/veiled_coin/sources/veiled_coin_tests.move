...
module veiled_coin::veiled_coin_tests {
    ...
    fun set_up_for_veiled_coin_test(
        ...
        print(&veiled_coin::clamp_u64_to_u32_amount(coin::balance<coin::FakeMoney>(signer::address_of(sender))));
        if (signer::address_of(recipient) != signer::address_of(sender)) {
            println(b"Recipient balance (as u64):");
            print(&coin::balance<coin::FakeMoney>(signer::address_of(recipient)));
            println(b"Sender balance (as u32):");
            print(&veiled_coin::clamp_u64_to_u32_amount(coin::balance<coin::FakeMoney>(signer::address_of(recipient))));
        } else {
    ...
    fun veil_test(
        ...
        veiled_coin::veil_to<coin::FakeMoney>(&sender, signer::address_of(&recipient), 150u32);
        ...
        assert!(coin::balance<coin::FakeMoney>(signer::address_of(&sender)) == veiled_coin::cast_u32_to_u64_amount(350u32), 1);
        assert!(veiled_coin::verify_opened_balance<coin::FakeMoney>(
    ...
    fun unveil_test(
        ...
        assert!(veiled_coin::total_veiled_coins<coin::FakeMoney>() == veiled_coin::cast_u32_to_u64_amount(150), 1);
        ...
        assert!(veiled_coin::total_veiled_coins<coin::FakeMoney>() == veiled_coin::cast_u32_to_u64_amount(100), 1);
    ...
    fun basic_viability_test(
        ...
        assert!(veiled_coin::verify_opened_balance<coin::FakeMoney>(signer::address_of(&sender), 150, &ristretto255::scalar_zero(), &sender_pk), 1);
        assert!(veiled_coin::verify_opened_balance<coin::FakeMoney>(signer::address_of(&recipient), 0, &ristretto255::scalar_zero(), &recipient_pk), 1);
