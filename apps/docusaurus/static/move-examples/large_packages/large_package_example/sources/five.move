...
/// * Directly [run a localnet](../nodes/localnet/run-a-localnet.md) using either the [Aptos-core source code](../nodes/localnet/run-a-localnet.md#using-the-aptos-core-source-code) or a [Docker image](../nodes/localnet/run-a-localnet.md#using-docker). These paths are useful for testing changes to the Aptos-core codebase or framework, or for building services on top of the Aptos blockchain, respectively.
/// * [Install the Aptos CLI](../tools/install-cli/index.md) and 2) start a [local node with a faucet](../nodes/localnet/using-cli-to-run-a-localnet.md#starting-a-localnet-with-a-faucet). This path is useful for developing on the Aptos blockchain, debugging Move contracts, and testing node operations.
///
