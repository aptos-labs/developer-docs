...
- `#[randomness]` is a required attribute for the execution engine to perform some safety check and enable the API for the transaction.
  ...
  - It also means randomness API calls are supported only in transactions that call an entry function.
...
you can build aptos-cli from source to support your randomness-dependent contract development.
...
### Security warning: the randomness is public!
