...
2. 打开一个新的 shell 终端。
3. 从启动本地网络的终端中复制 **Aptos 根密钥路**，并用它替换下方的命令中的 `mint-key-file-path`。
4. 运行以下命令启动水龙头：
...
此时，你将拥有一个位于 `0x1` 的特殊地址的账户，该账户具备执行代币铸造的能力。为了更深入了解，你可以跟随例如 https://learn.aptoslabs.com/tutorials 上的一些教程继续学习。
