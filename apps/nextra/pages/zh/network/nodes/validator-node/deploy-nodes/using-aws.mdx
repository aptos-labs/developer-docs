...
和 [AWS CLI](https://aws.amazon.com/cli/)。本指南假设您已经设置了 AWS 账户。
</Callout>
...
要连接到不同的 Aptos 网络，如测试网，请确保您下载了您想要连接的网络的正确创世区块和路标文件。同样，您需要修改 Terraform 文件以使用正确的配置（例如，`source`，`image_tag` 和 `chain_id`）。
</Callout>
...
您现在已经使用 AWS 完成了验证器和 VFN 的设置。请继续进行[连接节点](../connect-nodes.mdx)的操作。
