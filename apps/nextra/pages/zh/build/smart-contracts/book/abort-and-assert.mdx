...
[`return`](functions.mdx) and `abort` are two control flow constructs that end execution, one for
the current function and one for the entire transaction.

More information on [`return` can be found in the linked section](functions.mdx)
...
Similar to [`return`](functions.mdx), `abort` is useful for exiting control flow when some
