...
请注意，当您在主网上发布时，您传递到可选参数如 `--named-addresses` 中的凭据需要反映该网络上的账户，而不是测试凭据。
...
另外请注意，当发布 Move 模块时，如果一个包中有多个模块，则该包中的所有模块必须使用相同的账户。如果它们使用不同的账户，则发布将在交易级别失败。

另外，请注意，在发布 Move 模块的过程中，若一个软件包中有多个模块，则这些模块都必需使用同一个账户进行发布。不然，如果使用不同账户，则该软件包的发布操作将在交易处理的阶段遭遇失败。

您可以使用 [Gas Profiler](../../move/move-on-aptos/gas-profiling.mdx)估算与发布 Move 合约相关的 Gas 费用。{/* TODO: 修复这个链接 */}
...
对于您希望在单元测试之外保证代码按预期工作的情况，您可以使用 [Move Prover](../../smart-contracts/learn-move/prover.mdx)来形式化验证您的 Move 合约代码。

您可以通过按照[这些步骤](./setup-cli/install-move-prover.mdx)来安装 Move Prover。

安装 Move Prover 后，您可以通过运行来使用它从 Aptos CLI：
