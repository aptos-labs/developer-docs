...
<Callout type="info">
This tutorial is not meant to teach you the fundamentals of Move. That is a longer topic best learned through the [Move Book](https://aptos.dev/en/build/smart-contracts/book). If you need more analogies to familiar concepts like OOP, Solidity, or driving, check out the Developers Journey blog.
</Callout>

## 1. Setup

All transactions on Aptos require an account paying the decentralized network fee ("gas") to write information to the blockchain (our goal, publishing Your First Move Module, is therefore a transaction too!). In order to compile, test and deploy Move contracts, we need to have the Aptos CLI and create an account.

You can do that by following these steps:

1. Ensure you have [installed the Aptos CLI](https://aptos.dev/en/build/cli).
2. Open a new terminal.
3. Run `aptos --version` to verify you have it installed.
...
4. Create a new folder for this tutorial by running:

```bash filename="Terminal"
mkdir my-first-module
```

5. Run `cd my-first-module` to go into your new folder.
6. Run `aptos init` and press 'enter' for each step of setup to create a test account on `devnet`.
...
1. Copy the address from the command line for your new account.
...
2. Go to the [Aptos Explorer](https://explorer.aptoslabs.com/).
...
3. Switch to the `Devnet` network by clicking the "Mainnet" dropdown and selecting `Devnet`

4. Paste your newly created address into the search bar.

<Callout type="warning">
Do not press enter! There is a known bug where searching with Enter does not work.
</Callout>

5. Wait for the results to appear, then click the top result.
6. You should see your newly created account and a transaction with the faucet function, funding it with devnet tokens.
7. Click the "Coins" tab to see that you have 1 APT of the Aptos Coin. This will allow you to publish and interact with smart contracts on the aptos devnet.
...
Now that we have our environment set up and an account created, let's write and compile our first Move module. Unlike other blockchains where contracts exist independently, Move ties everything to accounts - both modules and their resources. Let's start with a simple example to understand the core concepts.
...
1. Initialize a new move project with `aptos move init --name my_first_module`

This creates a project structure with a `sources` directory and a `Move.toml` file.

2. Create a new file `sources/message.move` with our module code:

```move filename="sources/message.move"
...
3. Compile the Move module we just created with `aptos move compile --named-addresses my_first_module=default`
...
2. Create our tests: a new file `sources/message_tests.move` with:

```move filename="sources/message_tests.move"
#[test_only]
...
3. Now run the tests with `aptos move test --named-addresses my_first_module=default`
...
1. Publish your module with `aptos move publish --named-addresses my_first_module=default`
...
2. Type `y` and press Enter to confirm the transaction.
...
2. Search for your account address. You should notice that there is a new transaction in your account, the `code::publish_package_txn` function.
3. Click the "Coins" tab to see that you now have less than 1 APT of the Aptos Coin.

You have spent a small amount on gas to deploy the contract so should have around `0.99855 APT` remaining.

4. Look under the "Modules" tab
5. You should see your "message" module listed
...
1. Set a message `aptos move run --function-id 'default::message::set_message' --args 'string:Hello, Aptos!'`
...
2. After confirming with `y`, you should get a success response like:
...
3. View your stored message by checking under Resources on the Explorer.

4. We did it!

<Callout type="info">
How long did it take you to get through this guide?
We want to hear from you, please fill out [this short survey](https://forms.gle/Awi2AqxM5qR2SbD57)!
</Callout>
...
1. Try modifying the module to add a new feature. You can use the [Move Book](https://aptos.dev/en/build/smart-contracts/book) to build your understanding of writing Move modules.
2. To understand how Move works on-chain, you can learn about Move's [resource system](https://aptos.dev/en/network/blockchain/resources).
3. If you're building an application to interact with contracts or look up data from on-chain, learn how to use the SDKs [here](https://aptos.dev/en/build/sdks).
