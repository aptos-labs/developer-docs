...
In the fifth and final chapter of the tutorial on [building an end-to-end dapp on Aptos](../build-e2e-dapp.mdx), you will add functionality to the app so the user interface is able to handle cases where an account has created a list.

We have covered how to [fetch data](3-fetch-data-from-chain.mdx) (an account’s todo list) from chain and how to [submit a transaction](4-submit-data-to-chain.mdx) (new todo list) to chain using Wallet.
