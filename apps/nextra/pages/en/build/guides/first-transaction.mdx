...
<Callout type="info">
The transaction hash is a unique identifier for your transaction, similar to a tracking number for a package. You can use it to check the status of your transaction on the [Aptos Explorer](https://explorer.aptoslabs.com/).
</Callout>
</Steps>
...
## 8. Understanding the Transaction Lifecycle
...
## 9. Transaction Structure in Detail

Every Aptos transaction contains several important components:

- **Sender**: The account address initiating the transaction
- **Sequence Number**: A counter that increases with each transaction from the sender, preventing replay attacks
- **Payload**: The operation to perform (entry function call, script, or module publishing)
- **Max Gas Amount**: The maximum computational units the transaction can consume
- **Gas Unit Price**: The price per unit of gas in octas
- **Expiration Timestamp**: When the transaction becomes invalid if not executed
- **Chain ID**: Identifies which Aptos network the transaction is for (e.g., devnet, testnet, mainnet)

When a transaction is signed, the signature is combined with the transaction data to create a `SignedTransaction` that can be submitted to the blockchain.

<Callout type="info">
All transaction data is serialized using Binary Canonical Serialization (BCS) before being sent to the blockchain. This ensures consistent, compact representation of data across different platforms.
</Callout>

## 10. Advanced Transaction Features

Aptos supports several advanced transaction features that extend beyond basic transfers:

### Multi-Agent Transactions

Multi-agent transactions allow multiple accounts to participate in a single transaction. This is useful for operations that require approval from multiple parties.

```ts filename="multi-agent-example.ts"
// Example of building a multi-agent transaction
const transaction = await aptos.transaction.build.multiAgent({
  sender: alice.accountAddress,
  secondarySignerAddresses: [bob.accountAddress],
  data: {
    function: "0x1::your_module::your_function",
    functionArguments: [...],
  },
});
```
...
Sponsored transactions allow one account to pay gas fees for another account's transaction. This is useful for applications that want to cover gas costs for their users.

```ts filename="sponsored-example.ts"
// Example of building a sponsored transaction
const transaction = await aptos.transaction.build.simple({
  sender: alice.accountAddress,
  withFeePayer: true,  // Indicates this will be a sponsored transaction
  data: {
    function: "0x1::aptos_account::transfer",
...
Batch transactions allow you to submit multiple transactions quickly from a single account. This is useful for operations that require multiple independent steps.

```ts filename="batch-example.ts"
...
await aptos.transaction.batch.forSingleAccount({
  ...
  data: [
    ...
    {
      function: "0x1::aptos_account::transfer",
      functionArguments: [carol.accountAddress, 2000],
    },
  ],
});
```
...
1. **Learn about more complex transactions**:
   - [Multi-Agent Signatures](../sdks/ts-sdk/building-transactions/multi-agent-transactions.mdx) - Transactions requiring multiple signers
   - [Sponsoring Transactions](../sdks/ts-sdk/building-transactions/sponsoring-transactions.mdx) - Having another account pay gas fees
   - [Batching Transactions](../sdks/ts-sdk/building-transactions/batching-transactions.mdx) - Sending multiple transactions efficiently
   - [Binary Canonical Serialization (BCS)](../sdks/ts-sdk/building-transactions/bcs-format.mdx) - Understanding the serialization format
...
2. **Explore smart contracts**:
   ...
   - [Your First NFT](your-first-nft.mdx) - Create a non-fungible token

3. **Dive deeper into Aptos**:
   - [Account Basics](../../network/blockchain/accounts.mdx)
   - [Gas and Transaction Fees](../../network/blockchain/gas.mdx)
...
```python filename="multi-agent-example.py"
# Example of building a multi-agent transaction
# First create a transaction payload
...
# Get sequence number for the primary signer (Alice)
account_data = await rest_client.account(alice.address())
sequence_number = int(account_data["sequence_number"])

# Build and sign the transaction with multiple signers
# This requires more complex handling with the Python SDK
# See the Aptos Python SDK documentation for detailed examples
```
...
```python filename="sponsored-example.py"
# Example of a sponsored transaction approach in Python
# The sponsorship pattern requires more complex handling in the Python SDK
# See the Aptos Python SDK documentation for detailed examples
```
...
```python filename="batch-example.py"
# Example of batching transactions in Python
# Create multiple transaction payloads
payload1 = EntryFunction.natural(
    "0x1::aptos_account",
    "transfer",
    [],
    [TransactionArgument(bob.address(), Serializer.struct), TransactionArgument(1000, Serializer.u64)],
)

payload2 = EntryFunction.natural(
    "0x1::aptos_account",
    "transfer",
    [],
    [TransactionArgument(carol.address(), Serializer.struct), TransactionArgument(2000, Serializer.u64)],
)

# Submit transactions sequentially
# Note: The Python SDK doesn't have a built-in batching mechanism like the TS SDK
# You would need to handle each transaction individually
```
