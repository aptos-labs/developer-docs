...
Like all types with [`drop`](abilities.mdx) in Move, all integer types support the ["equal"](equality.mdx) and ["not equal"](equality.mdx) operations. Both arguments need to be of the same type. If you need to compare integers of different types, you will need to [cast](#casting) one of them first.
...
For more details see the section on [equality](equality.mdx)
