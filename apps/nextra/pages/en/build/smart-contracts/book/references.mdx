...
For more details on the rules of references, see [Structs and Resources](structs-and-resources.mdx)
...
[`copy` ability](abilities.mdx) as reading the reference creates a new copy of the value. This rule
...
[`drop` ability](abilities.mdx) as writing to the reference will discard (or "drop") the old value.
...
The only other types currently that has subtyping are [tuples](tuples.mdx)
...
[copies](variables.mdx#move-and-copy), but equally strict in ensuring unique ownership of mutable
...
also true for values of types without the `store` [ability](abilities.mdx), but note that
...
[persistent global storage](global-storage-structure.mdx), which needs to serialize values to
...
references inside of structs that do not have the `store` [ability](abilities.mdx), but that would
