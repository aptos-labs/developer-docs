...
In addition, the `move_to<T>(&signer, T)` [global storage operator](global-storage-operators.mdx)
...
any operation whether it be through an explicit [`copy`](variables.mdx#move-and-copy) instruction
or through a [dereference `*`](references.mdx#reading-and-writing-through-references).
