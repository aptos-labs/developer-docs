...
When comparing [references](references.mdx), the type of the reference (immutable or mutable) does
...
the type must have [`drop`](abilities.mdx). Recall that without the
[`drop` ability](abilities.mdx), ownership must be transferred by the end of the function, and such
...
[`drop` ability](abilities.mdx) safety guarantees!
...
reference types have the [`drop` ability](abilities.mdx). For example
...
While a programmer _can_ compare any value whose type has [`drop`](abilities.mdx), a programmer
...
This code is perfectly acceptable (assuming `Foo` has [`drop`](abilities.mdx)), just not efficient.
