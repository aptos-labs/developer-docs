...
---
{/* TODO this one is a duplicate of the tutorial in the tutorials folder, it shoud be consolidated to one*/}
...
The `set_message` function modifies the `hello_blockchain` `MessageHolder` resource. A resource is a data structure that is stored in [global storage](book/structs-and-resources.mdx#storing-resources-in-global-storage). The resource can be read by querying the following REST API:
