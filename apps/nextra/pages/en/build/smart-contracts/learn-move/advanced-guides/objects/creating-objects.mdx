...
When you create an Object, it needs to be configured for your use case by using the special `ConstructorRef`. These configurations can only happen when you construct the object. Later on it is impossible to change these settings.

Creating an Object involves two steps:

1. Creating the `ObjectCore` resource group.
2. Defining how the Object will behave using Refs.
...
Once you create your object, you will receive a `ConstructorRef` you can use to generate `Ref`s. `Ref`s can be used in future to enable / disable / execute certain Object functions such as transferring resources, transferring the object itself, or deleting the Object.
...
By default all Objects are transferrable. This can be changed via a `TransferRef` which you can generate with `object::generate_transfer_ref`.
