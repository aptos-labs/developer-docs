...
<Callout type="info">
    This function takes `Option<FunctionInfo>` as inputs.  Any functions that are provided `option::none()` will use the default behavior.  The `constructor_ref` must be for your fungible asset metadata object at creation time.
</Callout>
