...
For users using `primary_fungible_store` to manage assets, no change is required to interact with dispatchable fungible assets. This functionality will be automatically invoked.

For users using secondary `FungibleStore`s to interact with assets, `fungible_asset::withdraw` and `fungible_asset::deposit` will fail on assets that have the hook function registered. Instead use  `dispatchable_fungible_asset::withdraw` or `dispatchable_fungible_asset::deposit`. 

The `dispatchable_fungible_asset` versions are a drop-in replacement and work with functions that do not have hooks registered.
