...
In order to prevent re-entrancy prevention, there are some limitations:
- Always use `with_ref` in your custom hooks instead of `deposit` or `withdraw`:
    - `fungible_asset::deposit_with_ref`
    - `fungible_asset::withdraw_with_ref`
- Avoid calling into functions defined in `dispatchable_fungible_asset` and `primary_fungible_store`.  Inline functions are okay, other functions may cause errors on transfer.
