...
You will need this to set which account will have the privilege to create a collection. Make sure to set up the account on the same network as your template project.
...
VITE_COLLECTION_CREATOR_ADDRESS=0x71e3cdf1ef86d575ae38c7c2d19f22b8c88eb2b15082691782e5b8c8da401568
```
...
Or, if you use a different account, set the `VITE_COLLECTION_CREATOR_ADDRESS` to the account you sign in with.
...
`create-aptos-dapp` provides an npm command to easily deploy the static site to [Vercel](https://vercel.com/home). At the root of the folder, simply run npm run deploy and follow the prompts. Please refer to Vercel docs to learn more about the [Vercel CLI](https://vercel.com/docs/cli)

If you are looking for different services to deploy the static site, `create-aptos-dapp` utilizes Vite as the development tool, so you can follow the [Vite deployment guide](https://vitejs.dev/guide/static-deploy). In a nutshell, you would need to:
