...
Or, if you use a different account, set the `VITE_CREATOR_ADDRESS` to the account you sign in with.

<Callout type="warning">
Do not use the "Google Account" option if it is available. The account generated from that will not work as the `VITE_CREATOR_ADDRESS`. Use a wallet created with an Aptos wallet provider such as [Petra](https://petra.app/docs/use#create-a-new-account).
</Callout>
...
3. Choose a mint limit per account.
