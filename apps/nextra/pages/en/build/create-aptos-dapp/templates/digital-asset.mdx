...
1. Create assets for a collection of NFTs.
2. Publish the contract to create the collection on-chain.
3. Actually publish your collection.
...
<Steps>
### Create an `asset` folder for your collection.
...
### Add an image for the collection itself in the `asset` folder.

### Create a folder called `images` in the `asset` folder.

### Add all potential NFT images to the `images` folder.

These images must have the same file extension for this template app to work. 

### Create a folder called `metadata` in the `asset` folder.
...
<Steps>
### Prepare an account, or create one with an Aptos wallet like [Petra](https://petra.app/docs/use#create-a-new-account).
You will need this to run the contract to create a collection. Make sure to make the account on the same network as your template project.

### Use the below command to create the contract to mint your collection.

```bash npm2yarn
npm run move:init
```

The CLI will prompt you to provide the secret for your wallet in order for the tool to make the collection on your behalf.

This will create the contract that handles minting your collection, as well as setting the `VITE_MODULE_ADDRESS` in the `.env` file to indicate which account will own any collections that get minted.

### In the `.env` file, set `VITE_CREATOR_ADDRESS` to be the same as your account address.
...
### Run the below command to publish the contract.
...
Use the same account that you set in `.env` for the `VITE_CREATOR_ADDRESS`. 
Or, if you use a different account, set the `VITE_CREATOR_ADDRESS` to the account you end up signing in with.

### In the app, go to the "Create Collection" page (if not on that page already)

### Upload the `assets` folder you created previously.
If you do not have an `assets` folder or other properly formatted Colelction data, see [the beginning of these steps](#create-an-asset-folder-for-your-collection) for how to make one.

### Choose the required fields.
1. Choose the mint start date to determine when users can start minting NFTs.
2. Choose the mint end date to determine when users can no longer mint NFTs.
3. Choose a limit per account.

The rest are optional fields to help customize your minting.
...
You can use Collections that you have minted using the tool by going to the "My Collections" page and copying their address.
...
2. Custom flows for after someone mints a NFT (or even token gated experiences).
...
Creating a collection on follows the same flow as creating a Collection on testnet, but we need to change some configuration changes.
...
5. Check: open `.aptos/config.yaml` file and see that you have a profile under the `mainnet` name. In addition, open the `.env` file and check the `VITE_MODULE_ADDRESS` value is the same as the mainnet profile's account address.
...
7. Create or get the account you want to create a collection with, open the `.env` file and assign the account address as the `VITE_CREATOR_ADDRESS` value.
