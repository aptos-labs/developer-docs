...
The template utilizes React as the frontend framework and Next.js as the development tool, and is styled with [Tailwind CSS](https://tailwindcss.com/) and [shadcn/ui](https://ui.shadcn.com/).
...
1. Create a new database or a new branch on Neon Postgres for production and update the `DATABASE_URL` in the `.env` file of frontend.
...
**WARNING**: Do not ever try to backfill the data, as logic like point calculation is incremental. If you backfill (e.g. processing same event twice), you will get wrong point data. So please always revert all migrations and re-index from the first tx your contract deployed.
