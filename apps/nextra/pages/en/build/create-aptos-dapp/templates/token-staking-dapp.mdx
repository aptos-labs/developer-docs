...
## Quick Start
- [Create a stake on chain](#publish-the-move-contract)
- [Create an incentivize pool](#create-an-incentivize-pool)
...
Or, if you use a different account, set the `VITE_CREATOR_ADDRESS` to the account you sign in with and republish the contract.

### Click the `Add Incentivize` button

Once you are connected with the same account that you set in `.env` for the `VITE_CREATOR_ADDRESS`, you should be able to see the `Incentivize Pool` component with a `Add Incentivize` button.
...
6. Create or get the account you want to create a asset with, open the `.env` file and assign the account address as the `VITE_CREATOR_ADDRESS` value.
6. Create or get the asset you want to use in your dapp, open the `.env` file and assign the asset address as the `VITE_FA_ADDRESS` value.
7. Finally, run `npm run move:publish` to publish your move module on Aptos mainnet.
8. The next step would be to create an incentivize pool using this account. Simply follow [Create an incentivize pool](#create-an-incentivize-pool)
