...
Compiler helps with test and abort attacks, requiring functions using randomness to be private.  However, the randomness API currently does not prevent undergassing attacks.  The smart contract will need to be written in a certain way to avoid it.
</Callout>
...
### Keep undergassing attacks in mind

<Callout type="warning">
**The randomness API currently does not prevent undergassing attacks.** Carefully read the undergassing section to understand about undergassing attacks and how to prevent them. As a dApp developer, you will need to design applications using randomness with safety in mind.
</Callout>
...
Formally, this is referred to as an [undergassing attack](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-41.md#undergassing-attacks),
...
<Callout type="Warning">
  **WARNING: randomness API currently does not prevent undergassing attacks.**
  As a dApp developer, you need to be very careful in your design to avoid this type of attack.
  Here are some ideas of how to prevent undergassing attack generally.
  ...
  Note that everything that does not fall in above categories can be susceptible to undergassing attack in a subtle ways. Reach out if you need help.

  We will be providing more functionality in the future, to allow for more complex code to be able to be safe against undergassing attacks.
</Callout>
