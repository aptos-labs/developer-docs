...
- `#[randomness]` is a required attribute for the execution engine to perform some safety check and enable the API for the transaction.
  ...
  - It also means randomness API calls are supported only in transactions that call an entry function.
    For example, calling the API from a script won't work.
...
1. abort if the result is not in their favor.
   ...
   This is referred to as a [_test-and-abort attack_](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-41.md#test-and-abort-attacks).

The Aptos Move compiler have been updated to prevent this attack for your contract safety:
...
A malicious user can control its account balance so it covers at most 108 gas units (or set transaction parameter `max_gas=108`), and the punishing branch will never be reached.
