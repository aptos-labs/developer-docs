...
A malicious user can control its account balance so it covers at most 108 gas units (or set transaction parameter `max_gas=108`), and the punishing branch will never succeed.
