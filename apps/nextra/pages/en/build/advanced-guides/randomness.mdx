...
Compiler helps with test and abort attacks, requiring functions using randomness to be private.  However, the randomness API current does not prevent undergassing attacks.  The smart contract will be need to be written in a certain way to avoid it.
</Callout>
...
**The randomness API currently doesn’t prevent undergassing attacks.** Carefully read the undergassing section to understand about undergassing attacks and how to prevent them. As a dApp developer, you will need to design applications using randomness with safety in mind.
</Callout>
...
- Make your entry function gas independent from the randomness outcome.
  The simplest example is to not "act" on the randomness outcome, i.e. read it and store it for later. Note that calling any other functions  can have variable gas costs. For example, when calling randomness to decide which player should win, and then depositing the winnings to the winner might seem like a fixed gas cost. But, `0x1::coin::transfer` / `0x1::fungible_asset::transfer` can have a variable cost based on the user's onchain state.
