...
The `set_message` function modifies the `hello_blockchain` `MessageHolder` resource. A resource is a data structure that is stored in [global storage](../move/book/structs-and-resources.mdx#storing-resources-in-global-storage). The resource can be read by querying the following REST API:
