...
This new way of indexing uses the [Transaction Stream Service](../txn-stream.mdx). You can either use the [Labs-Hosted Transaction Stream Service](../aptos-hosted.mdx) or [run your own instance of Transaction Stream Service](../self-hosted.mdx).
...
For each processor you're migrating, you'll need to create a config file using the template below. You can find more information about each field of the config file [here](../self-hosted.mdx#configuration).
...
If you choose to, you can run a self-hosted instance of the Transaction Stream Service and connect your processors to it. Instructions to run a Self-Hosted Transaction Stream can be found [here](../self-hosted.mdx).
...
If you'd like to run the processor as a Docker image, the instructions are listed [here](../self-hosted.mdx#run-with-docker).
