...
Although this tutorial is written using Python, the modern recommendation is to use Rust if possible to write your custom processors as that language has more support in the ecosystem. The high level steps are still the same across these two languages.
</Callout>
...
Processors consume transactions from the Transaction Stream Service. In order to use the Labs-Hosted Transaction Stream Service you need an authorization token. Follow [this guide](../aptos-hosted.mdx#authorization-via-api-key) to guide to get a token from the Developer Portal. Create an API Key for `Testnet`, as this tutorial is for `Testnet`. Once you're done, you should have a token that looks like this:
...
```python filename="python/utils/processor_name.py"
...
```python filename="utils/worker.py"
...
```python filename="schema_names.py"
EXAMPLE = "example"
