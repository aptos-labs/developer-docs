...
- [Source Code](https://github.com/aptos-labs/aptos-ts-sdk)
- [Reference Docs](https://aptos-labs.github.io/aptos-ts-sdk/@aptos-labs/ts-sdk-latest)

# Quickstart

0. Install the TypeScript SDK using the package manager of your choice:
...
You can send transactions to change the state of the ledger. Transactions let you send tokens like APT, trigger Move modules, trade NFTs, and more. To begin with though, here’s how you can send a basic transaction to transfer APT.
...
Once you have built the transaction, you must sign and submit it. Signing with the sending account verifies that you own or manage that account. The client will then automatically determine the cost to execute this transaction based on network load.

Here we use `aptos.signAndSubmitTransaction` which combines these two steps into one:
...
Lastly, you can run `aptos.waitForTransaction` to guarantee your code executes after the transaction has been processed and applied. This also helps you get any errors that may occur after submitting, such as the transaction being rejected. 
...
2. Looking up data from on-chain using client helper functions like [`aptos.getAccountModules`](https://aptos-labs.github.io/aptos-ts-sdk/@aptos-labs/ts-sdk-1.12.2/classes/Aptos.html#signAndSubmitTransaction).
3. Submitting a transaction by building, signing, submitting.
4. Waiting for Aptos to execute the transaction.

To see all this in action, you can copy and run the [full working code snippet for this quickstart](https://github.com/aptos-labs/aptos-ts-sdk/blob/main/examples/typescript/simple_transfer.ts).

# Further Learning

Explore more in-depth guides for each of the common SDK workflows:

1. [Fetching Data](./ts-sdk/fetching-data.mdx)
2. [Building and Sending Transactions](./ts-sdk/building-and-sending-transactions.mdx)
3. [Testing With a Local Network](./ts-sdk/testing-with-a-local-network.mdx)
