...
### (Optional) Install the plugins plugins for any “Legacy Standard Compatible” Wallets you want to support from [this list](https://github.com/aptos-labs/aptos-wallet-adapter/blob/main/README.md#supported-wallet-packages).
...
The legacy standard required installing plugins manually. For wallets that have not updated to using the AIP-62 standard, their plugins must be installed and passed in to the `Provider` manually.

For example:

```bash filename="Terminal"
npm install @bitget-wallet/aptos-wallet-adapter
```
...
2. Include `plugins` for any wallets that are using the legacy adapter.
