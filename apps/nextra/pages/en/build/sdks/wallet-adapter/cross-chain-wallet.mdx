...
Thanks to [AIP-113 Derivable Account Abstraction](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-113.md), we can manage cross-chain signatures flexibly and securely on the Aptos network.
...
When a user enters a dApp that supports cross-chain wallets, the interaction and experience feel the same as with any Aptos wallet.
The user connects with a cross-chain wallet (e.g., Phantom for Solana) and can view their Derivable Abstracted Account (DAA) details, sign messages, and submit transactions to the Aptos chain.

When a dapp submits a transaction using a cross-chain wallet, the wallet adapter utilizes the `signIn` function (defined in the cross-chain wallet standard) for domain verification and security.
...
### Hoe does DAA work in a cross chain wallet?

When a user connects to a dApp using the cross-chain wallet adapter, the adapter computes the user's Derivable Abstracted Account (DAA) address and converts the cross-chain wallet to follow the Aptos wallet standard interface.
...
- `authenticationFunction`: This is a function that exists on-chain and is used to verify the signature of the cross-chain wallet.
- `accountIdentity`: This represents the identity of the account used in the on-chain authentication function to verify the signature of the cross-chain wallet.
In the Wallet Adapter, the `accountIdentity` is based on the original cross-chain wallet’s public key and the dApp domain (e.g., mydomain.com). The format is:
`${originWallet.publicKey}${domain}`

<Callout>
   Since the account identity is based on the dApp domain, it is scoped to the dApp context. As a result, each wallet has a different DAA address on different dApps.
</Callout>


### How to integrate cross-chain wallets in my dApp?

Make sure you integrate with the Aptos Wallet Adapter by following these [steps](./dapp.mdx)

Supporting cross-chain wallets in a dApp requires only a 2-step installation process.
...
That will handle the logic and implementation to include the cross-chain wallets as if they were Aptos wallets.
...
In most cases, allowing users to submit a transaction with a cross-chain wallet to the Aptos chain requires using a sponsor transaction. This is because the cross-chain wallet might not have APT to pay for gas.
...
### Considerations
- Since the wallet is a cross-chain wallet and is most likely not integrated with Aptos, simulation is not available in the wallet.
...
- While the cross-chain wallet prioritizes DAA, each wallet also retains the original wallet, so developers should be able to use it and interact with it
