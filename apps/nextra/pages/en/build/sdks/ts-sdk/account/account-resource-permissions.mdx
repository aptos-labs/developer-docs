...
Account Resource Permissions (ARP) on Aptos allow users to grant specific permissions to decentralized applications (dapps), enabling them to access limited resources on the user's behalf.

<Callout>
 Visit the [Account Resource Permissions Example Repository](https://github.com/aptos-labs/account-resource-permissions-example) for a complete example of how to use account resource permissions with the TS-SDK.
</Callout>
...
Dapps can revoke permissions by calling the `revokePermissions` function. This will remove the permissions for the given public key.
