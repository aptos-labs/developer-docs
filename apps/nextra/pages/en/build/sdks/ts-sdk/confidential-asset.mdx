...
# Interact with Confidential Coin (CA) via the typescript SDK

You can use `confidentialCoin` property of `Aptos` client to interact with `CA`

### Initialization

Operations in CA require generating zk-proofs (ZKPs), and depending on your environment, you need to define `Range Proof` calculation.
...
CA implements the Pollard's Kangaroo method for solving the discrete logarithm problem (DLP)
...
Notice, you can see, that both `preloadTables` and `preloadTablesForBalances` aiming for the same thing.
As far as SDK's decryption method is abstract - you could implement it by yourself.
And `preloadTablesForBalances` provides a slightly faster approach with concurrent decryption execution.
...
To interact with confidential token user must create a [unique key pair](/en/build/smart-contracts/confidential-coin#confidential-asset-store).
...
To interact with confidential tokens, a user has to [register](/en/build/smart-contracts/confidential-coin#register) their encryption key (EK) in contracts
...
Check is user has already registered with token.
...
This method returns you the user's [`pending` and `actual`](/en/build/smart-contracts/confidential-coin#confidential-asset-store) confidential balances, and to [decrypt](/en/build/smart-contracts/confidential-coin#encryption-and-decryption) them, you can use `ConfidentialAmount` class
...
User can't operate with `pending` balance, so you could [rollover](/en/build/smart-contracts/confidential-coin#rollover-pending-balance) it to `actual` one.

And to do so - use `aptos.confidentialCoin.rolloverPendingBalance`.

<Callout type="warning">
  Important note, that user's actual balance need to be [normalized](/en/build/smart-contracts/confidential-coin#normalize) before `rollover` operation.
</Callout>
...
Usually you don't need to explicit call [normalization](/en/build/smart-contracts/confidential-coin#normalize)
...
To [withdraw](/en/build/smart-contracts/confidential-coin#withdraw) your assets out from confidential balance:
...
For [transfer](/en/build/smart-contracts/confidential-coin#confidential-transfer) you need to know recipient's encryption key and `aptos` account address
...
To do [key rotation](/en/build/smart-contracts/confidential-coin#rotate-encryption-key), you need to create new decryption key and use `aptos.confidentialCoin.rotateCBKey`
