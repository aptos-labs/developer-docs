...
Or use [`pepper`](/en/build/guides/aptos-keyless/how-keyless-works) from [Keyless Account](/en/build/guides/aptos-keyless)
...
Let's say you have recipient's account address, lets get his Encryption key.
...
<Callout type="warning">
  ...
  In that case we could do a `rollover` with `freeze` option, to move assets from pending balance to actual one and lock our balance.
...
Now let's create a new decryption key and rotate out balance:
