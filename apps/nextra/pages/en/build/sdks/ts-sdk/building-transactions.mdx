...
Transactions allow you to change on-chain data or trigger events. Generally, transactions follow this flow:
...
Building a transaction is how we specify:

1. The `sender` account. This user normally pays the gas fees for this transaction. See [Transaction Sponsoring](./building-transactions/sponsoring-transactions.mdx) to learn how to have another account pay for fees.
2. The `function` being called on-chain. This is the identifier for the entry function on-chain that will trigger when we execute this transaction.
...
```tsx filename="build-a-transaction.ts"
...
Transactions have a couple of additional features which let them be very flexible:
