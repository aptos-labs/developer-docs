...
Normally, the account that is executing a transaction pays for the gas fees. You can allow another account to cover those charges though by sponsoring a transaction.

This can be used to help manage fees from a central account when setting up several accounts with smart contracts. That way you will not have to worry about having to “refill” intermediary accounts with funds.

## How To Sponsor a Transaction

1. When **building** the transaction, include the parameter `withFeePayer: true`.
    
    ```tsx filename="sponsor.ts"
...
2. To **sign** the transaction:
    1. Sign with the sender account using `.sign`.
    2. Also sign with the sponsor account using `.signAsFeePayer`.
...
3. (Optional) When **simulating** the transaction, include the parameter `feePayerPublicKey: account.publicKey`
...
4. Then **submit** the transaction by combining both signatures like so:
...
5. **Waiting** is the same as for a normal transaction.
