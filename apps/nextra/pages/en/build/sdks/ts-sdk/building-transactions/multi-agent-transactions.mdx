...
<Callout type="info">
Anywhere in the [simple transaction flow](../building-transactions.mdx) that used to use `.simple` now uses `.multiAgent`
</Callout>
...
### Simulate the transaction using the parameter `secondarySignersPublicKeys`.
...
### Lastly, wait for the transaction to resolve like so.
