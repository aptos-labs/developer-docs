...
- [Validator nodes (VNs)](validator-nodes.mdx) - participates in consensus and drives [transaction processing](txns-states.mdx).
...
- [Public fullnodes (PFNs)](fullnodes.mdx) - run by someone who is not a validator operator, PFNs cannot connect directly to a validator node and therefore rely upon VFNs for synchronization.
- [Archival nodes (ANs)](../nodes/configure/state-sync.mdx#archival-pfns) - is a fullnode that contains all blockchain data since the start of the blockchain's history.
