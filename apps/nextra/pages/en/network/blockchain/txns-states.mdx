...
- **State**: The blockchain state represents the accumulation of the output of execution of transactions, the values stored within all [resources](resources.mdx).
- [**Events**](events.mdx): Ancillary data published by the execution of a transaction.

<Callout type="info">
Only transactions can change the blockchain state.
</Callout>
...
A transaction may end in one of the following statuses:
...
<Callout type="info">
See [Aptos Blockchain Deep Dive](../blockchain.mdx) for a comprehensive description of the Aptos transaction lifecycle.
</Callout>
...
The Aptos blockchain's state, or global state, represents the persisted storage that's accessible from smart contracts in the Aptos blockchain. Each validator node in the blockchain must know the latest version of the global state to execute any transaction.

Anyone can submit a transaction to the Aptos blockchain to modify the state. Upon execution of a transaction, a transaction output is generated. A transaction output contains zero or more operations to manipulate the ledger state called **write sets** emitting a vector of resulting events, the amount of gas consumed, and the executed transaction status.
...
## Transactions change the global state
