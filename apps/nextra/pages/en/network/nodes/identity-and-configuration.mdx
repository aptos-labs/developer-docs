...
The following command is used to generate the above key and identity YAML files. 
See, for example, [Step 10 while using AWS to deploy the validator node](./validator-node/deploy-nodes/using-aws.mdx), 
or in [Step 10 while using GCP](./validator-node/deploy-nodes/using-gcp.mdx).
