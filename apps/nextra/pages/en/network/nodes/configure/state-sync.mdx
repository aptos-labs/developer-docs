...
- **Testnet**: To sync the entire blockchain history, restore from a [backup](../bootstrap-fullnode/aptos-db-restore.mdx). Otherwise, download [a snapshot](../bootstrap-fullnode.mdx) or use [fast sync](state-sync.mdx#fast-syncing).
