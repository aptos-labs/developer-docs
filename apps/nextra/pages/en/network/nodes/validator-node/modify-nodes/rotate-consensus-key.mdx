...
Among all the identidies, the node finds the one that appears on-chain validator set, and uses it as the consensus key.
</Callout>
...
However, keys are never deleted automatically from the stroage.
