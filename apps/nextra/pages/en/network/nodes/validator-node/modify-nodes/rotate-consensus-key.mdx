...
or every X months as a common security practice.
...
```
<Callout type="info">
Don't delete `identity_blob_path`. It is a required field.

Field `overriding_identity_paths` is optional, and allows multiple identities to be specified.

Among all the identities, the node finds the one that appears on-chain validator set, and uses it as the consensus key.
</Callout>

<Callout type="info">
Old keys may still be available even if you updates `identity_blob_path`
or delete items from `overriding_identity_paths`,
due to the reasons below.

There is a separate **secure storage** (typically a file `secure_storage.json`) hosting all identities.
The content of `identity_blob_path` and `overriding_identity_paths` are added to the secure storage on node start, if missing.
However, keys are never deleted automatically from the storage.

To completely wipe out an old key, you will have to manually make changes to the secure storage.
See the end of the guide for more details.
</Callout>

Now restart the node.
It should be aware of the new key but continue consensus participation with the old key.
