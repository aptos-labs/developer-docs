...
> Beta: This documentation is in experimental, beta mode. Supply feedback by [requesting document changes](../../../community/site-updates.md#request-docs-changes). See also the related [Staking Pool Operations](./staking-pool-operations.mdx) instructions.

Validator operators should follow these instructions to carry out delegation pool operations for [delegated staking](../../../blockchain/learn-about-aptos/delegated-staking.mdx). You may delegate as little as 10 APT plus a small add stake fee that will be mostly refunded as rewards at the end of the current epoch. You might notice that some UIs might use 11 APT as the minimum for a round number. Note that your validator will become part of the _Active Validator Set_ only when the delegation pool satisfies the minimum cumulative [staking requirement of 1 million APT](./staking-pool-operations.mdx).
...
2. [Initialize local configuration and create an account](../../../../build/cli/cli-configuration.mdx) on the Aptos blockchain.
...
In the [Aptos TypeScript SDK](../../../../build/sdks/index.mdx), a View function request would resemble:
