...
This document describes how to perform [staking](../../../blockchain/learn-about-aptos/staking.mdx) pool operations. Note that a staking pool can only accept stake
...
View the values in [Aptos Blockchain Networks](../../../nodes/networks.mdx) to see how profiles can be configured based on the network.
</Callout>
...
<Callout type="info">
Before you proceed, see [Validation on the Aptos blockchain](../../../blockchain/learn-about-aptos/staking.mdx#validation-on-the-aptos-blockchain) for a brief overview.
</Callout>
...
<Callout type="info">
For testnet or devnet `--url` field values, see [Aptos Blockchain Networks](../../../nodes/networks.mdx).
</Callout>
...
- Successful leader-validator proposals during the current epoch. Also see [Validation on the Aptos blockchain](../../../blockchain/learn-about-aptos/staking.mdx#validation-on-the-aptos-blockchain) for the distinction between leader-validator and the voter-validator.
...
`DistributeEvent` is emitted when there is a transfer from staking_contract to the operator or staker (owner). Rewards can be tracked either by listening to `DistributeEvent` or by using the [View function](../../../../build/apis/index.mdx#reading-state-with-the-view-function) to call `staking_contract_amounts`. This will return `accumulated_rewards` and `commission_amount`.
