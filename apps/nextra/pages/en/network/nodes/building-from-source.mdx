...
[CLI Binary releases are available](../../build/cli/install-cli.mdx), but if you
...
See [Choose a network](../../build/advanced-guides/system-integrators-guide.mdx) for an
explanation of their differences.

<details>
<summary>Release Branches</summary>
<Tabs groupName="release-branches" height="175px">
  <Tabs.Tab value="devnet" defaultChecked={true}>
    ```bash filename="Terminal"
    git checkout --track origin/devnet
    ```
  </Tabs.Tab>
  <Tabs.Tab value="testnet">
    ```bash filename="Terminal"
    git checkout --track origin/testnet
    ```
  </Tabs.Tab>
  <Tabs.Tab value="mainnet">
    ```bash filename="Terminal"
...
skip the prover tests.  To install the prover (optional), follow the [Prover guide](../../build/smart-contracts/learn-move/prover.mdx)
