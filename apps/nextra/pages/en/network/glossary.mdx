...
See [Accounts](./blockchain/learn-about-aptos/accounts.mdx) for more information.
...
- An **Application Programming Interface (API)** is a set of protocols and tools
  ...
  nodes.
- See [documentation](../build/apis/fullnode-rest-api.mdx) for more details.
...
- **Byzantine Fault Tolerance** (BFT) is the ability of a distributed system to
  ...
  or “[Byzantine](#byzantine-validator),” validators below a certain threshold.
...
- **Command line interface** refers to the Aptos CLI used for developing on the
  ...
  at [Use the Aptos CLI](../build/cli/use-cli.md).
...
- You can use the faucet in a few different ways:
  - With the [Aptos CLI](../build/cli/use-cli/index.md).
  - Through a wallet, such
    as [Petra](https://aptosfoundation.org/ecosystem/project/petra)
    or [Pontem](https://aptosfoundation.org/ecosystem/project/pontem-wallet).
    See full list
    of [Aptos Wallets](https://aptosfoundation.org/ecosystem/projects/wallets).
  - Using an SDK, for example by using the `FaucetClient` in the TypeScript SDK.
  - With a direct HTTP request. Learn how to do
    this [here](../build/advanced-guides/system-integrators-guide.mdx#calling-the-faucet-other-languages). {/* TODO: Fix link */}
...
- A **fungible asset** is an asset, such as a currency, share, in-game resource,
  ...
  APT for another.
- Follow the [Asset Standards](../build/standards.mdx#asset-standards) to create
...
- A **fungible unit** is an individual unit of a fungible asset. These units are
  ...
  Octa (the smallest unit of APT) is a fungible unit.
...
See [Gas and Storage Fees](./network/learn-about-aptos/gas-txn-fee.mdx) for more information.
...
- Leaders are selected by a function that takes the
  current [round number](https://fb.quip.com/LkbMAEBIVNbh#ffYACAO6CzD) as input.
...
- The *
  *[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)
...
- It has two different kinds of code
  &mdash; [Move scripts](#transaction-or-move-script)
...
- Move is a safe and secure programming language for web3 that emphasizes access
  ...
  in [Move on Aptos](../build/move/move-on-aptos.mdx).
...
See [Resource accounts](../build/move/move-on-aptos/resource-accounts.mdx) for
instructions on use.

### REST Service

- The **REST Service** component is the external interface of an Aptos node. Any
...
- Method for decreasing latency for BFT protocols.
  See [paper](https://arxiv.org/pdf/2306.03058.pdf)
...
follow [Move scripts](../build/move/move-on-aptos/scripts/script-tutorial.mdx) and
the [Your First Multisig](../tutorials/first-multisig.md) tutorial. {/* TODO Tutorial link */}
