...
`_meta.ts` files. In the docs theme, there are some extra options available to
...
`_meta.ts` file as key-value pairs. For example, if you have the following
file structure:
...
You can define how the pages are shown in the sidebar via the `_meta.ts` file:

```json filename="_meta.ts"
{
  "index": "My Homepage",
  "contact": "Contact Us",
  "about": "About Us"
...
<Callout>
  If any routes are not listed in the `_meta.ts` file, they will be appended
...
The top-level `_meta.ts` file contains the meta information for the top-level
pages and folders:
...
And the nested `_meta.ts` file contains the meta information for pages in the
same folder:

```json filename="pages/fruits/_meta.ts"
{
  "apple": "Apple",
  "banana": "Banana"
}
...
move directories around without having to change the `_meta.ts` file.
...
Then Nextra knows that the `fruits` key in `_meta.ts` defines a folder with an
...
```json filename="pages/_meta.ts" {6-9}
{
  "index": "My Homepage",
  "contact": "Contact Us",
  "fruits": "Delicious Fruits",
...
```json filename="pages/_meta.ts" {9}
{
  "index": "My Homepage",
  "contact": "Contact Us",
  "fruits": "Delicious Fruits",
...
```json filename="pages/_meta.ts" {4}
{
  "index": "My Homepage",
  "contact": {
    "display": "hidden"
...
In your top-level `_meta.ts` file, you can set everything as a page, instead
...
<div className="mt-4 text-center text-sm">
  ...
  ↗](https://stackblitz.com/edit/nextra-2-docs-eszspq?file=pages%2F_meta.ts)
</div>
...
<div className="mt-8 text-center text-sm">
  [Live example on StackBlitz
  ↗](https://stackblitz.com/edit/nextra-2-docs-2qopvp?file=pages%2F_meta.ts)
</div>
...
```json filename="pages/_meta.ts" {2-4}
{
  "*": {
    "type": "page"
  },
...
<Callout>
  To customize the look of titles and separator lines in the sidebar use JSX
  elements.
</Callout>
...
```json filename="pages/_meta.ts" {5}
{
  "index": {
    "title": "Home",
    "theme": {
...
<div className="mt-4 text-center text-sm">
  [Live example on StackBlitz
  ↗](https://stackblitz.com/edit/nextra-2-docs-hg77h3?file=pages%2F_meta.ts,pages%2Findex.mdx)
</div>
