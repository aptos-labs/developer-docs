...
Move言語では、パッケージを使用してコードを整理し、依存関係を管理します。パッケージは、関連するモジュールとその依存関係をまとめたものです。

## パッケージの構造

典型的なMoveパッケージは以下のような構造を持ちます：

```
my_package/
├── Move.toml          # パッケージマニフェスト
├── sources/           # ソースファイル
│   ├── module1.move
│   └── module2.move
└── tests/             # テストファイル
...
`Move.toml`ファイルは、パッケージの設定と依存関係を定義します：
...
パッケージ内のモジュールで使用する名前付きアドレスを定義できます：
...
他のパッケージを依存関係として追加する方法：

```toml
[dependencies]
# ローカルパッケージ
MyLocalPackage = { local = "../my-local-package" }

# Gitリポジトリからのパッケージ
MyGitPackage = { git = "https://github.com/user/repo.git", rev = "main" }

# 公開レジストリからのパッケージ
MyRegistryPackage = { addr = "0x1" }
```

## パッケージのビルド

パッケージをビルドするには、`aptos move compile`コマンドを使用します：
...
テストを実行するには、`aptos move test`コマンドを使用します：
...
module counter_addr::counter_tests {
    ...
    #[test]
    fun test_counter() {
        let account = signer::address_of(@counter_addr);
...
パッケージを公開するには、以下の手順を実行します：

1. パッケージをビルドしてテスト
2. `Move.toml`の設定を確認
3. `aptos move publish`コマンドを使用して公開
...
Recall that Move has [named addresses](address.mdx) and that
